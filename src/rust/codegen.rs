...
// The x87 inline path.
//
// A register the generated code writes holds the double's bits in its
// mantissa word and FPU_RELAXED_TAG in its exponent word; the helpers turn
// that back into the 80-bit format whenever they read a register. An
// operation whose operands all carry the tag is done here, as one double
// operation; any other falls to the helper it always called. What the inline
// path gives up: the stack-fault and status-word bookkeeping of the helpers.
// Measured on a title in a match before this: one call per x87 operation,
// with a ten-byte value passed through memory, was 8.5% of the page's time.
...
/// below eight by everything that writes it, so st(0) needs no mask.
/// `gen_fpu_tag_ok` wants the index too -- deriving it back out of the
/// address costs more than keeping it, since every caller of the address
/// already has (or can cheaply have) the index on hand.
fn gen_fpu_st_index_addr_from(
    ctx: &mut JitContext,
    top: &WasmLocal,
    i: u32,
) -> (WasmLocal, WasmLocal) {
    ctx.builder.get_local(top);
    if i != 0 {
        ctx.builder.const_i32(i as i32);
        ctx.builder.add_i32();
    ...
    ctx.builder.add_i32();
    let addr = ctx.builder.set_new_local();
    (index, addr)
}

/// The physical index and byte address of st(i) in the register file, both
/// in locals.
...
/// marked empty, and its tag word reads the marker generated code writes.
/// A pop leaves the tag word of the register it emptied unchanged -- real
/// hardware leaves a popped register's bits standing too (fsave-style
/// introspection, and this suite's own register comparisons, still read
/// them), so clearing it on every writer is not an option and the empty bit
/// stays part of the answer. What earlier cost here was recovering the
/// index from the address (`(addr - fpu_st) >> 4`) purely to read that bit;
...
fn gen_fpu_apply_f64_binop(ctx: &mut JitContext, op: FpuFastBinOp) {
    match op {
        ...
        FpuFastBinOp::Sub | FpuFastBinOp::SubR => ctx.builder.sub_f64(),
        FpuFastBinOp::Div | FpuFastBinOp::DivR => ctx.builder.div_f64(),
    }
}
...
/// The two operands in the order the operation wants them: the reversed
/// forms take st(0) on the right.
fn gen_fpu_binop_operands(
    ctx: &mut JitContext,
    op: FpuFastBinOp,
    st0_addr: &WasmLocal,
    other: &mut dyn FnMut(&mut JitContext),
) {
    if matches!(op, FpuFastBinOp::SubR | FpuFastBinOp::DivR) {
        other(ctx);
...
pub fn gen_fpu_binop_sti(
    ...
    dbg_assert!(target_sti == 0 || target_sti == sti);
    if !crate::jit::fpu_inline_enabled() {
    ...
    ctx.builder.free_local(op_index);
    ctx.builder.free_local(st0_addr);
    ctx.builder.free_local(st0_index);
    ctx.builder.const_i32(target_sti as i32);
    gen_fpu_get_sti(ctx, sti);
...
/// back, the slot marked full, the bits and the tag stored.
...
// A load from memory takes the arm on what the arithmetic arms take, a normal
// or a zero; anything else goes through the library, which is where an SNaN
...
// format rather than tagged. The test is on the bits: |x| less the smallest
// normal is below the span of the normals.

/// Pushes whether the single whose bits are in `bits` is a normal or a zero.
fn gen_fpu_f32_bits_ok(ctx: &mut JitContext, bits: &WasmLocal) {
    ctx.builder.get_local(bits);
    ctx.builder.const_i32(1);
    ctx.builder.shl_i32();
    ctx.builder.const_i32(1);
    ctx.builder.shr_u_i32();
    let abs = ctx.builder.set_new_local();
    ctx.builder.const_i32(0x7F00_0000);
    ctx.builder.get_local(&abs);
    ctx.builder.const_i32(0x0080_0000);
    ctx.builder.sub_i32();
    ctx.builder.gtu_i32();
    ctx.builder.get_local(&abs);
    ctx.builder.eqz_i32();
    ctx.builder.or_i32();
    ctx.builder.free_local(abs);
}

/// Pushes whether the double whose bits are in `bits` is a normal or a zero.
fn gen_fpu_f64_bits_ok(ctx: &mut JitContext, bits: &WasmLocalI64) {
    ctx.builder.get_local_i64(bits);
    ctx.builder.const_i64(1);
    ctx.builder.shl_i64();
    ctx.builder.const_i64(1);
    ctx.builder.shr_u_i64();
    let abs = ctx.builder.set_new_local_i64();
    ctx.builder.const_i64(0x7FE << 52);
    ctx.builder.get_local_i64(&abs);
    ctx.builder.const_i64(-(1 << 52));
    ctx.builder.add_i64();
    ctx.builder.gtu_i64();
    ctx.builder.get_local_i64(&abs);
    ctx.builder.const_i64(0);
    ctx.builder.eq_i64();
    ctx.builder.or_i32();
    ctx.builder.free_local_i64(abs);
}

/// `fld m32`: a normal or zero single widened to a double and pushed tagged,
/// any other pushed by the helper.
...
/// `fld m64`: a normal or zero double pushed tagged as it is, any other
/// pushed by the helper.
...
/// `fld st(i)`: a tagged register copied inline, any other pushed by the helper.
pub fn gen_fpu_push_sti(ctx: &mut JitContext, sti: u32) {
    if !crate::jit::fpu_inline_enabled() {
        gen_fpu_get_sti(ctx, sti);
        ctx.builder.call_fn2_i64_i32("fpu_push");
        return;
...
/// `fst m32` / `fstp m32`: a tagged st(0) narrowed and stored inline.
pub fn gen_fpu_store_m32(ctx: &mut JitContext, modrm_byte: ModrmByte, pop: bool) {
    gen_modrm_resolve(ctx, modrm_byte);
    let address_local = ctx.builder.set_new_local();
    if crate::jit::fpu_inline_enabled() {
        let done = ctx.builder.block_void();
...
/// `fst m64` / `fstp m64`: a tagged st(0) stored inline as it is.
pub fn gen_fpu_store_m64(ctx: &mut JitContext, modrm_byte: ModrmByte, pop: bool) {
    gen_modrm_resolve(ctx, modrm_byte);
    let address_local = ctx.builder.set_new_local();
    if crate::jit::fpu_inline_enabled() {
        let (st0_index, st0_addr) = gen_fpu_st_index_addr(ctx, 0);
...
/// The double on the stack into a register that already carries the tag: the
/// result of every inline form goes into a register whose tag the form read.
