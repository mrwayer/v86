; The stack top and the empty mask crossing a helper in the middle of a block:
; a helper that reads them where generated code left them, a helper that
; moves them and an inline form that must see the move. Every value here is
; exact in a double except the 80-bit one, which no double holds and so keeps
; every form that reads it on its helper arm.
...
%include "header.inc"
    ...
    ; one register left so the x87 state is what the fixture compares
    fld dword [esp]                       ; 1.5, top 7
